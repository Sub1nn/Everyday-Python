...
class ShareSansarScrapper:
    ...
    def parse_headers(self, soup: BeautifulSoup) -> list:
        t_heads = soup.find_all("thead")  # ['<thead>...</thead>',...]
        headers = []  # ['sno', 'symbol', 'conf','open',..., '_120_days',...]
        for t_head in t_heads:
            for th in t_head.find_all("th"):
                th = th.text.replace("%", "percent")
                heading = slugify(th)  # Diff % => Diff percent => diff-percent
                heading = heading.replace("-", "_")  # diff-percent => diff_percent
                # 120 Days => 120_days
                if not heading.isidentifier():
                    heading = "_" + heading  # 120_days => _120_days
                headers.append(heading)
        return headers

    def parse_rows(self, soup: BeautifulSoup, headers: list) -> list[dict]:
        # headers = ['s_no', 'symbol', 'conf','open',..., '_120_days',...]
        t_bodies = soup.find_all("tbody")
        today_prices = []
        for t_body in t_bodies:
            for tr in t_body.find_all("tr"):
                td_data = {}
                for i, td in enumerate(tr.find_all("td")): # 1, symbol
                    if headers[i] == "symbol":
                        td_data.update(
                            {
                                headers[i]: td.a.text, # symbol: ACLBSL
                                "name": td.a.get("title"), # name: Aarambha Chautari Laghubitta Bittiya Sanstha Limited
                            }
                        )  
                    else:
                        td_data.update({headers[i]: td.text}) 
                # {"sno":1, "symbol":"ACLBSL", "name":"Aarambha Chautari Laghubitta Bittiya Sanstha Limited", "conf":39.00}
    ...
    def save_as_json(self, FILE_NAME:str, today_prices: list[dict]):
    ...
    def parse_data(self, soup: BeautifulSoup):
        headers = self.parse_headers(soup)  # ['s_no', 'symbol',...]
        today_prices = self.parse_rows(soup, headers)  # list
        df = self.format_data(today_prices) # list
